...
\begin{definition}[特征值，特征向量]
	设 $A$ 是 $n$ 级矩阵，如果存在数 $\lambda$ 和非零列向量 $\vbf{\alpha}$ 使得
	\[ A \vbf{\alpha} = \lambda\vbf{\alpha} \]
	那么称 $\lambda$ 是 $A$ 的一个特征值，称 $\vbf{\alpha}$ 是 $A$ 的属于特征值 $\lambda$ 的一个特征向量，称 $f(\lambda) = |\lambda E - A|$ 是 $A$ 的特征多项式。
\end{definition}
