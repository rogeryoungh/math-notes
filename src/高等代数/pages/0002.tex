...
\begin{theorem}
	设 $n$ 级矩阵 $A$ 依据给定排列 $i_1i_2\cdots i_n$ 对行指标进行重排得到 $A_1$，则 $A_1$ 的行列式为
	\[ |A_1| = (-1)^{\tau(i_1i_2\cdots i_n)} |A| \]
\end{theorem}

\begin{proof}
	设一列元素 $a_{1j_1}a_{2j_2}\cdots a_{nj_n}$ 经过 $s$ 次相邻互换变为 $a_{i_1k_1}a_{i_2k_2}\cdots a_{i_nk_n}$，则有
	\[ \begin{aligned}
...
同理，给定列指标的一个排列 $k_1k_2\cdots k_n$ 和对应的矩阵 $A_2$，则 $n$ 级矩阵的行列式为
\[ |A_2| = (-1)^{\tau(k_1k_2\cdots k_n)} |A| \]
...
\begin{theorem}
	方程组有唯一解的充要条件是 $|A| \neq 0$。定义 $B_j$ 为矩阵 $A$ 第 $j$ 列换为常数项后的矩阵，此时解为
	\[ \left( \frac{|B_1|}{|A|}, \cdots, \frac{|B_n|}{|A|} \right) \]
\end{theorem}

\begin{proof}
	假设行列式有唯一解，由上一章知矩阵 $\widetilde{A}$ 可以通过初等行变换变化为阶梯型矩阵 $J$，$|A| = l |J| \neq 0$。
\end{proof}
